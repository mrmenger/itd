...
function loadDB() {
  try {
    if (fs.existsSync(STORAGE_FILE)) {
      ...
      if (db && db.users) { console.log(`✅ DB loaded`); return db; }
    }
  } catch (e) { console.error('read err:', e.message); }
  try {
  ...
  } catch (e) { console.error('env err:', e.message); }
...
app.patch('/api/auth/me', auth, async (req, res) => {
  ...
  if (newPassword && password) {
    ...
    if (newPassword.length < 6) return res.status(400).json({ success: false, message: 'Мин. 6 символов' });
