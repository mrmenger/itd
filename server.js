...
function loadDB() {
  ...
  try {
    if (process.env.ITD_DATA) {
      ...
      if (db && db.users) { saveFile(db); return db; }
    }
  } catch(e) {}
...
function saveFile(db) {
  try { fs.writeFileSync(STORAGE_FILE, JSON.stringify(db), 'utf-8'); } catch(e) {}
}
...
setInterval(() => {
  ...
  if (!url) return;
  try { https.get(url + '/api/ping', ()=>{}).on('error', ()=>{}); } catch(_) {}
}, 14 * 60 * 1000);
...
app.patch('/api/auth/me', auth, async (req, res) => {
  ...
  persist(); res.json({ success: true, data: safe(user) });
});
...
app.patch('/api/posts/:id/like', auth, (req, res) => {
  ...
  persist(); res.json({ success: true, data: { liked: idx===-1, likesCount: post.likes.length }});
});
...
app.delete('/api/posts/:id', auth, (req, res) => {
  ...
  DB.posts.splice(idx,1); persist(); res.json({ success: true });
});
...
app.patch('/api/users/me/follow/:id', auth, (req, res) => {
  ...
  persist(); res.json({ success: true, data: { following: !already, followersCount: target.followers.length }});
});
...
app.get('/api/messages', auth, (req, res) => {
  ...
  const set = new Set();
  DB.messages.forEach(m => { if(m.from===myId) set.add(m.to); if(m.to===myId) set.add(m.from); });
  const dialogs = [];
  set.forEach(pid => {
    const partner = DB.users.find(u => u.id===pid); if(!partner) return;
...
app.delete('/api/messages/msg/:msgId', auth, (req, res) => {
  ...
  DB.messages.splice(idx,1); persist(); res.json({ success: true });
});
