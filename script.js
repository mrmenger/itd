...
function loadSession() {
  ...
  if (t && u) {
    try { S.token = t; S.user = JSON.parse(u); return true; } catch {}
  }
  return false;
}
...
function buildColorSwatches(containerId, initial, onSelect) {
  const wrap = $(containerId);
  wrap.innerHTML = '';
  COLORS.forEach((c, i) => {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'cs' + (c===initial?' active':'');
...
$('regForm').addEventListener('submit', async e => {
  ...
  $('regErr').textContent     = '';
  $('rNameErr').textContent   = '';
  $('rUserErr').textContent   = '';
...
function buildPostCard(p, container='feed') {
...
// Delegation — postsList AND profilePosts
  ...
  $(listId).addEventListener('click', async e => {
    ...
    if (delBtn) {
      ...
      try {
        ...
        // remove from all rendered lists
...
function renderSidebarStats() {
  ...
  $('sPosts').textContent    = myPosts.length;
  $('sFollowers').textContent= followers;
  $('sFollowing').textContent= following;

  $('rPosts').textContent    = myPosts.length;
  $('rLikes').textContent    = fmt(myLikes);
  $('rFollowers').textContent= followers;
}
...
['hPostBtn','sidePostBtn','qcTrigger','emptyPostBtn'].forEach(id=>{
  const el=$(id); if(el) el.onclick=openPostModal;
});
...
$('postTa').addEventListener('keydown', e=>{
  if((e.ctrlKey||e.metaKey)&&e.key==='Enter'&&!$('publishBtn').disabled){e.preventDefault();submitPost();}
});
...
function renderPeople(filter='') {
  ...
  if(filter){
    ...
    filtered=S.users.filter(u=>u.displayName.toLowerCase().includes(lf)||u.username.toLowerCase().includes(lf));
  }
...
$('peopleList').addEventListener('click', async e=>{
  ...
  if(followBtn){
    ...
    try {
      ...
      if(data.following){
        if(!S.user.following.includes(tid)) S.user.following.push(tid);
      } else {
        S.user.following=S.user.following.filter(i=>i!==tid);
      }
      ...
      if(card&&tu){
        ...
        card.querySelector('.person-card__stats').textContent=`${pc} постов · ${tu.followers.length} читателей`;
      }
      renderSidebarStats();
      toast(data.following?`Вы читаете @${tu?.username}`:`Отписались от @${tu?.username}`,'success');
    } catch(err){ toast(err.message,'error'); }
    return;
  }
  // Message from people page
  ...
  dialogs.forEach(d=>{
    ...
    div.innerHTML=`
      ...
      <div class="dialog-item__info">
        ...
        <div class="dialog-item__last">${esc(lastText.slice(0,50))}</div>
      </div>
...
function updateMsgBadge(n) {
  [$('msgBadge'),$('msgBadgeSide')].forEach(el=>{
    if(!el) return;
...
async function openChat(userId) {
  ...
  switchPage('messages');
  // Show chat panel, hide dialogs on mobile
...
// Poll for new messages every 5 seconds when on messages page
...
async function openProfile(userId) {
  ...
  if(!user){
    try{ const {data}=await api('GET',`/users/${userId}`); user=data; } catch{ toast('Пользователь не найден','error'); return; }
  }
  ...
  posts.forEach(p=>profilePosts.appendChild(buildPostCard(p,'profile')));
}
...
function toggleEditProfile(show, user) {
  ...
  // Build emoji grid
  buildEmojiGrid('editEmojiGrid', em=>{
    S.editEmoji=em;
  });
  // Mark current
  $('editEmojiGrid').querySelectorAll('.emoji-btn').forEach(b=>{
    b.classList.toggle('sel', b.textContent===S.editEmoji);
  });
...
// Nav delegation
document.addEventListener('click', e=>{
  const link=e.target.closest('.nav__link, .side-nav__item');
  if(!link) return;
