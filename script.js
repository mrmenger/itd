...
const S = {
  ...
  bgEnabled:true,bgAnimFrame:null,bubbles:[],
};
...
function relTime(iso){const s=Math.floor((Date.now()-new Date(iso))/1000);if(s<60)return'только что';if(s<3600)return`${Math.floor(s/60)} мин. назад`;if(s<86400)return`${Math.floor(s/3600)} ч. назад`;if(s<604800)return`${Math.floor(s/86400)} дн. назад`;return new Date(iso).toLocaleDateString('ru-RU',{day:'numeric',month:'long'})}
...
function loadSession(){const t=localStorage.getItem(TOKEN_KEY),u=localStorage.getItem(USER_KEY);if(t&&u){try{S.token=t;S.user=JSON.parse(u);return true}catch{}}return false}
...
function applyTheme(theme){
  ...
  initBubbles(); // re-init bubbles with correct colors
}

function applyAccent(color){
  document.documentElement.style.setProperty('--primary',color);
  const d=shadeColor(color,-20);
  document.documentElement.style.setProperty('--primary-d',d);
  document.documentElement.style.setProperty('--primary-glow',color+'55');
  document.documentElement.style.setProperty('--primary-bg',color+'18');
...
function applySize(scale){
  ...
  $$('.size-btn').forEach(b=>b.classList.toggle('active',parseFloat(b.dataset.size)===scale));
}
...
function loadSettings(){
  ...
  const size=parseFloat(localStorage.getItem(SIZE_KEY)||'1');
  applySize(size);
  const bg=localStorage.getItem(BG_KEY);
  S.bgEnabled=bg!=='false';
  const toggle=$('bgToggle');
  if(toggle) toggle.checked=S.bgEnabled;
}

/* ══════════ ANIMATED BUBBLES ══════════ */
function getBubbleColors(){
  const style=getComputedStyle(document.documentElement);
  const primary=style.getPropertyValue('--primary').trim()||'#6C63FF';
  const theme=document.documentElement.getAttribute('data-theme')||'light';
  if(theme==='dark') return[primary,primary+'99',primary+'55','#A78BFA','#A78BFA88'];
  if(theme==='purple') return[primary,primary+'aa','#D946EF','#D946EF88','#A855F766'];
  return[primary,primary+'88',primary+'44','#A78BFA','#A78BFA55'];
}
...
function animateBubbles(){
  if(!S.bgEnabled){
    ...
    S.bgAnimFrame=null;return;
  }
  const canvas=$('bubbleCanvas');
  if(!canvas){S.bgAnimFrame=requestAnimationFrame(animateBubbles);return;}
  ...
  S.bubbles.forEach(b=>{
    ...
    ctx.lineWidth=3+Math.sin(b.phase)*1.5;
    ctx.globalAlpha=0.5+Math.sin(b.phase*1.3)*0.2;
    ctx.stroke();

    // inner fill with gradient
    const grad=ctx.createRadialGradient(b.x-rr*0.2,b.y-rr*0.2,rr*0.1,b.x,b.y,rr*0.85);
    grad.addColorStop(0,b.color+'88');
    grad.addColorStop(0.5,b.color+'44');
    grad.addColorStop(1,'transparent');
    ctx.beginPath();
    ctx.arc(b.x,b.y,rr*0.85,0,Math.PI*2);
    ctx.fillStyle=grad;
    ctx.globalAlpha=0.35+Math.sin(b.phase)*0.12;
    ctx.fill();

    // shine
    ctx.beginPath();
    ctx.arc(b.x-rr*0.28,b.y-rr*0.28,rr*0.18,0,Math.PI*2);
  ...
  S.bgAnimFrame=requestAnimationFrame(animateBubbles);
}

function startBubbles(){
  if(S.bgAnimFrame) cancelAnimationFrame(S.bgAnimFrame);
  initBubbles();
  S.bgAnimFrame=requestAnimationFrame(animateBubbles);
}
...
function buildColorSwatches(containerId,current,onSelect){
  ...
  COLORS.forEach(c=>{
    ...
    btn.className='cs'+(c===current?' active':'');btn.style.background=c;btn.dataset.color=c;
...
$('prevAv').style.background=S.selColor;$('miniAv').style.background=S.selColor;
...
$('regForm').addEventListener('submit',async e=>{
  ...
  try{const{data}=await api('POST','/auth/register',{displayName,username,password,bio,emoji:S.selEmoji,color:S.selColor});saveSession(data.token,data.user);await bootApp();toast('Добро пожаловать! 🎉','success');}
...
function syncHeaderUI(){
  ...
  const els=[['hAv',u.emoji,u.color],['sideAv',u.emoji,u.color],['mobAv',u.emoji,u.color],['bottomNavAv',u.emoji,u.color],['qcAv',u.emoji,u.color],['modalAv',u.emoji,u.color]];
  els.forEach(([id,em,cl])=>{const el=$(id);if(el){el.textContent=em;el.style.background=cl;}});
  [['umenuName',u.displayName],['umenuHandle','@'+u.username],['sideName',u.displayName],['sideHandle','@'+u.username],['mobName',u.displayName],['mobHandle','@'+u.username],['modalName',u.displayName],['modalHandle','@'+u.username]].forEach(([id,val])=>{const el=$(id);if(el)el.textContent=val;});
}
...
async function fetchPosts(userId=null){
  try{const url=userId?`/posts?userId=${userId}`:'/posts';const{data}=await api('GET',url);if(!userId)S.posts=data;return data;}
  ...
  art.innerHTML=`
    ...
    <div class="pc-foot">
      <button class="post-act post-act--like ${liked?'liked':''}" data-like="${esc(p.id)}">
        <svg width="17" height="17" viewBox="0 0 24 24" fill="${liked?'currentColor':'none'}" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>
...
function setupPostDelegation(listId){
  $(listId).addEventListener('click',async e=>{
    ...
    if(delBtn){
      if(!confirm('Удалить пост?'))return;const id=delBtn.dataset.del;
      try{
        await api('DELETE',`/posts/${id}`);S.posts=S.posts.filter(p=>p.id!==id);
        document.querySelectorAll(`.post-card[data-id="${id}"]`).forEach(c=>{c.style.transition='opacity .3s,transform .3s';c.style.opacity='0';c.style.transform='scale(.96)';setTimeout(()=>{c.remove();renderSidebarStats();},300);});
...
function renderSidebarStats(){
  ...
  [['sPosts',myPosts.length],['sFollowers',followers],['sFollowing',following],['rPosts',myPosts.length],['rLikes',fmt(myLikes)],['rFollowers',followers],['mobPosts',myPosts.length],['mobFollowers',followers],['mobFollowing',following]].forEach(([id,val])=>{const el=$(id);if(el)el.textContent=val;});
}
...
$('postTa').addEventListener('input',()=>{
  ...
  const cc=$('charN').parentElement;cc.className='char-c';if(len>800)cc.classList.add('warn');if(len>950)cc.classList.add('danger');
...
/* ══════════ PEOPLE ══════════ */
async function fetchUsers(){try{const{data}=await api('GET','/users');S.users=data;}catch(err){console.error(err);}}
...
$('peopleList').addEventListener('click',async e=>{
  ...
  if(followBtn){
    ...
    try{
      ...
      if(data.following){if(!S.user.following.includes(tid))S.user.following.push(tid);}else S.user.following=S.user.following.filter(i=>i!==tid);
...
async function renderDialogs(){
  const dialogs=await fetchDialogs();const list=$('dialogsList');list.innerHTML='';
  const unreadTotal=dialogs.reduce((s,d)=>s+d.unread,0);updateMsgBadge(unreadTotal);
  if(dialogs.length===0){$('dialogsEmpty').classList.remove('hidden');return;}
  $('dialogsEmpty').classList.add('hidden');
  dialogs.forEach(d=>{
    const p=d.partner;const div=document.createElement('div');
    div.className='dialog-item'+(S.chatPartnerId===p.id?' active':'');div.dataset.uid=p.id;
    div.innerHTML=`
...
function buildBubble(m){
  const isOut=m.from===S.user.id;const div=document.createElement('div');
  div.className=`msg-bubble msg-bubble--${isOut?'out':'in'}`;div.dataset.msgId=m.id;
...
async function sendMessage(){
  ...
  try{const{data}=await api('POST',`/messages/${S.chatPartnerId}`,{text});S.chatMessages.push(data);$('chatInput').value='';$('chatInput').style.height='auto';renderChatMessages();renderDialogs();}
  catch(err){toast(err.message,'error');}
...
$('chatMessages').addEventListener('click',async e=>{
  ...
  if(!confirm('Удалить сообщение?'))return;const id=btn.dataset.delMsg;
...
async function openProfile(userId){
  ...
  if(!user){try{const{data}=await api('GET',`/users/${userId}`);user=data;}catch{toast('Пользователь не найден','error');return;}}
  ...
  if(isMe){
    ...
    fb.onclick=async()=>{try{const{data}=await api('PATCH',`/users/me/follow/${userId}`);fb.classList.toggle('following',data.following);fb.textContent=data.following?'Читаю':'+ Читать';$('pFollowers').textContent=data.followersCount;if(data.following){if(!S.user.following.includes(userId))S.user.following.push(userId);}else S.user.following=S.user.following.filter(i=>i!==userId);localStorage.setItem(USER_KEY,JSON.stringify(S.user));renderSidebarStats();}catch(err){toast(err.message,'error');}};
...
$('saveEditBtn').onclick=async()=>{
  ...
  try{const{data}=await api('PATCH','/auth/me',payload);S.user={...S.user,...data};localStorage.setItem(USER_KEY,JSON.stringify(S.user));syncHeaderUI();$('editProfileCard').classList.add('hidden');$('editCurPass').value='';$('editNewPass').value='';openProfile(S.user.id);toast('Профиль обновлён ✅','success');}
...
function initSettingsUI(){
  ...
  if(bgToggle){
    bgToggle.addEventListener('change',()=>{
      S.bgEnabled=bgToggle.checked;
      localStorage.setItem(BG_KEY,S.bgEnabled);
      if(S.bgEnabled)startBubbles();else{if(S.bgAnimFrame)cancelAnimationFrame(S.bgAnimFrame);S.bgAnimFrame=null;const canvas=$('bubbleCanvas');if(canvas){const ctx=canvas.getContext('2d');ctx.clearRect(0,0,canvas.width,canvas.height);}}
    });
  }

  // Accent colors
  $('accentColors').addEventListener('click',e=>{
    const btn=e.target.closest('.accent-btn');if(!btn)return;
    applyAccent(btn.dataset.accent);
  });

  // Size buttons
  $$('.size-btn').forEach(btn=>{
    btn.onclick=()=>applySize(parseFloat(btn.dataset.size));
  });

  // Settings page profile edit button
  const settingsEditBtn=document.querySelector('.settings-actions .btn--outline[data-page]');
  if(settingsEditBtn){settingsEditBtn.onclick=()=>{switchPage('profile-me');setTimeout(()=>toggleEditProfile(true,S.user),400);};}
}
...
function switchPage(page){
  ...
  const mpb=$('mobProfileBar');if(mpb)mpb.style.display=page==='feed'?'flex':'none';
...
document.addEventListener('click',e=>{
  ...
  if(!link)return;const page=link.dataset.page;if(!page)return;
...
(async function init(){
  ...
  if(loadSession()){
    try{const{data}=await api('GET','/auth/me');S.user=data;localStorage.setItem(USER_KEY,JSON.stringify(data));await bootApp();}
